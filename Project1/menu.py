...
def login():
    while True:
        ...
        print(f"[DEBUG] User entered: {choice}")  # 调试信息
        if choice == "1":
...
def grade_analysis_menu(student_id):
    while True:
        ...
        print(f"[DEBUG] User entered: {sub_choice}")  # 调试信息
        if sub_choice == "1":
            ...
            Grade.get_statistics(course_id)
        elif sub_choice == "0":
...
def teacher_menu():
    print("[DEBUG] Entering teacher_menu...")  # 调试信息
    while True:
        print("***** Teacher Menu *****")
        ...
        try:
            ...
            print(f"[DEBUG] An error occurred: {e}")  # 捕获所有异常以调试